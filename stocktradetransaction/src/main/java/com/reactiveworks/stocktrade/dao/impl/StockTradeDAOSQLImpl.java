...
/*		try {
			con.setAutoCommit(false);			//Beginning Transaction.
		} catch (SQLException sqlExp) {
			throw new DatabaseOperationFailureException("Cannot begin transaction.",sqlExp);
		}
*/
		...
		try {
			...
			try {
				...
				throw new DatabaseOperationFailureException("Cannot commit changes to database",sqlExp);
			}
			return rows;
			...
			try {
				con.rollback();   //Reverting changes made before error occured.
			} catch (SQLException sqlExp1) {
				LOGGER.error("Cannot rollback to initial stage.",sqlExp1);
				throw new DatabaseOperationFailureException("Cannot rollback to initial stage.",sqlExp1);
			}
	 ...
	 * @throws DatabaseOperationFailureException if any database operations lead to failure.
	 * @throws DatabaseAccessFailureException if any failure in database access.
	 */
	@Override
